...
class AgentEngine:
    def __init__(self, project_root: Path, settings: Settings):
        ...
        self.creative_media = CreativeMediaEngine(project_root, settings)
    ...
    def _mission_anchor_from_history(self, history: list[Any]) -> MissionAnchor | None:
        for item in reversed(history):
            ...
            if role != "system" or not content.strip().startswith(MISSION_ANCHOR_PREFIX):
                continue
            anchor = self._parse_mission_anchor(content)
    ...
    def _request_is_broad_mission_followup(self, message: str) -> bool:
        ...
        if not cleaned:
            return False
        if self._explicit_prompt_workspace(message):
            return False
    ...
    def _mission_aware_message(self, message: str, anchor: MissionAnchor | None) -> str:
        if anchor is None or not self._request_is_broad_mission_followup(message):
            return message
        context = self._mission_anchor_context(anchor)
        if not context:
            return message
        return (
            f"{message}\n\n"
    ...
    async def run(
        ...
        if prompt_requests_creative_media(request.message):
            return self._run_creative_media_request(
                request,
                mode=mode,
                workspace_root=workspace_root,
                workspace_files=workspace_files,
    ...
    def _run_creative_media_request(
        self,
        request: AgentRequest,
        *,
        mode: ModeName,
        ...
        if request.run_validation or self._request_asks_for_validation(request.message):
        ...
        size_update: dict[str, Any] = {}
        if media_kind in {"logo", "icon", "brand_kit"}:
            size_update = {"width": 1024, "height": 1024, "aspect_ratio": "1:1"}

        try:
            media_job = self.creative_media.create_job(
                MediaCreativeRequest(
        ...
        asset_lines = [
            ...
            for asset in media_job.assets[:8]
        ]
    ...
    def _harden_native_cpp_draft(
        ...
        has_cpp_entry = any(path.endswith((".cpp", ".cxx", ".cc", ".hpp", ".h")) for path in paths) or self._workspace_has_any(
            workspace_root,
            ("src/main.cpp", "main.cpp"),
        )
        has_cmake = "cmakelists.txt" in paths or (workspace_root / "CMakeLists.txt").exists()
        cpp_request = self._request_mentions_cpp_project(message) or has_cpp_entry or has_cmake

        if not cpp_request or not has_cmake or not has_cpp_entry:
            return draft
        removed_shell_scripts = self._drop_shell_only_cpp_validation_changes(draft)
        if "build.py" in paths or existing_build_py:
            self._prefer_python_build_command(draft)
        ...
        draft.changes.append(
            FileChange(
                ...
                content=self._cmake_build_py_content(),
            )
        )
        self._prefer_python_build_command(draft)
...
"""
    ...
    def _manifest_stack_family(self, manifest: WorkspaceProjectManifest | None) -> str:
        if manifest is None:
            return ""
        preset_id = self._manifest_contract_value(manifest, "preset_id") or manifest.preset_id
        stack_family = self._manifest_contract_value(manifest, "stack_family")
    ...
    def _draft_stack_families(self, draft: AgentDraft) -> set[str]:
        ...
        if any(path == "go.mod" or path.endswith(".go") for path in paths):
            families.add("go")
        if self._draft_has_desktop_host_surface(draft):
