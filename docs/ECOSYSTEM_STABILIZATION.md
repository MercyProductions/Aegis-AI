...
| Validation detection | Improved | Core now lists JS test/build validation commands only when matching package scripts exist, ignores project files in dependency/build folders, ignores damaged root build-marker directories, distinguishes native Visual Studio/C++ solutions from .NET solutions, and avoids duplicate default-command detection during validation runs. |
