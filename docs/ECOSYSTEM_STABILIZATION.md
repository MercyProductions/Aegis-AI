...
| Ollama model routing | Pass | Core detects `qwen3-coder:30b` with configured fallbacks available, and malformed or credential-like local URLs degrade to health/config defaults. |
...
- Website checkpoint restore and apply safety tests: 52 storage tests passed
