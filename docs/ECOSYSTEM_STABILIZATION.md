...
- Website backend Core bridge shared runtime envelope redaction tests after `/api/core-runtime` hardening: 20 passed
- Website backend Core bridge result redaction tests after default result-boundary hardening: 21 passed
...
- Hardened Website `/api/core-runtime` shared runtime responses so raw Core health, model, settings, memory, diagnostics, and dashboard envelopes are recursively redacted before they reach clients.
- Hardened Website Core bridge result construction so successful Core `data`, `envelope`, and `kind` metadata are redacted before any Website endpoint reuses them.
