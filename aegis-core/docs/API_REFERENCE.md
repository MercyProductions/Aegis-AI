...
Malformed provider responses, including invalid JSON or non-object OpenAI-compatible payloads, return bounded provider failures instead of leaking internal parser exceptions to clients. Provider and credential diagnostics redact common provider/OAuth secret fields such as `api_key`, `x-api-key`, `client_secret`, `access_token`, `refresh_token`, `private_key`, and `token` across JSON, query-string, and assignment-style errors while preserving actionable context. Bare `token:` parser diagnostics are preserved unless the value looks secret-like, so ordinary errors such as `unexpected token: <` remain readable.
