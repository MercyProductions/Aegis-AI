...
Desktop, Website, and VS Code backend/Core URL settings are normalized before use. Inputs like `127.0.0.1:8788`, pasted `/v1/ecosystem/dashboard` URLs, and legacy Core endpoints such as `/health` or `/models` are reduced to a clean Core base URL, while empty values fall back to the local defaults. Website and VS Code preserve reverse-proxy path prefixes such as `https://proxy.local/aegis` and append Core API paths under that prefix.
...
- Confirm the configured Ollama URL includes a valid host and port. `127.0.0.1:11434` is normalized to `http://127.0.0.1:11434`, and pasted paths such as `/api/tags` are trimmed to the base URL. VS Code preserves reverse-proxy prefixes such as `https://proxy.local/ollama`.
