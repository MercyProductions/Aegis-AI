...
Aegis Core is the consolidation layer for the Desktop App, Website, VS Code extension, and Visual Studio extension. It provides reusable Ollama integration, workspace scanning, memory, roadmap generation, validation detection, diagnostics, predictive change simulation, and plan-only agent workflows through a Python service layer, local FastAPI API, and optional `aegis` CLI.
